...
void chart::prinptinner(node* &cur, bool makeArray)
{
	...
	if(!cur->child&&attr[0]=='\0')
		{
			...
			if(makeArray) 
			{
				...
				if(data[0]=='\0')
					...
					cout<<tab(tabs)<<"\"#text\" : "<<brackets(trimString(data));

					
			}
			else if (data[0]!='\0' )cout<<" "<<brackets(trimString(cur->data));
			else cout<<" "<<"null";
	...
	else
	{
			...
			while(traveseAttributes->attr)
				{
					...
					cout<<tab(tabs)<<"\"@"<<trimString( traveseAttributes->attr->key)<<"\" : "<<trimString (traveseAttributes->attr->value);
					if(text[0]!='\0'||cur->child||traveseAttributes->attr->attr) cout<<",";
					cout<<endl;
		...
		if(text[0]!='\0')
		{
			cout<<tab(tabs)<<"\"#text\""<<" : "<<brackets(trimString (cur->text));
			if(cur->child) cout<<",";
			cout<<endl; 
		}
...
void chart::printouter(node* N)
{
	...
	cout<<tab(tabs)<<brackets(trimString (cur->tag))<<":";
	...
	// cout<<cur->child->child->attr->attr->key<<endl;
}
